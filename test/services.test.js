...
posts.forEach(function (post) {
  ...
  test(post, function (t) {
    ...
    t.equal(
      typeof metadata.controls,
      "object",
      "controls must be formatted as a list"
    );
    ...
    metadata.controls.forEach(function (control) {
      ...
      t.equal(
        control.model.substr(0, 5),
        "base.",
        'model must start with "base." not ' + control.model.substr(0, 5)
      );
      t.ok(control.include, "must have an include");
      t.ok(control.popover, "must have a popover");
      t.notEqual(
        forms.indexOf(control.include),
        -1,
        "include must be one of the following: " + forms.join(", ")
      );
      t.ok(control.label, "must have a label");
      if (control.include == "select")
        t.ok(control.options, "must have options");
      if (control.include == "input") {
        t.ok(control.type, "must have a type");
        t.notEqual(
          ["text", "number", "range"].indexOf(control.type),
          -1,
